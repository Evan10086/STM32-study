...
{	
	...
	int16_t alarmHour = 0x19;
	int16_t alarmMinute = 0x21 ;
	...
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_8;//第9 10号引脚
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出类型
...
//	
	...
	if (RTC_ReadBackupRegister(RTC_BKP_DR0) != 0x6668)
	{
		rtc_init();
		
		RTC_WriteBackupRegister(RTC_BKP_DR0,0x6668);
	}
	...
	while(1)
	{
		...
		if (RTC_TimeStructure.RTC_Hours == alarmHour &&
			RTC_TimeStructure.RTC_Minutes == alarmMinute &&
